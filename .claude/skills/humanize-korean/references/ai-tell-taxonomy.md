# AI 한글 티 분류 체계 v2.1 (Korean AI-Tell Taxonomy)
...
> **v2.1 추가 (2026-06-11):** Toss Technical Writing의 문장 중심 원칙 중 humanize 범위에 맞는 항목만 흡수. **본진 신규 1건** — `F-6` 무내용 작업 명사화·절차 동사 [S2]. 문서 목적·제목 품질·섹션 구조·개요 작성 같은 문서 설계 규칙은 범위 밖으로 제외.
>
> **v2.0 추가 (2026-05-07):** 한국 번역학계 8대 번역투 정통성 계보(이영옥 2001·김도훈 2009·김정우 2007·김혜영 2019 등) + 보고서 §III.3(8유형) 통합. **본진 신규 4건** — `A-16` 영어 대명사 직역 [S1] · `A-18` 관계절 좌향 수식 [S2] · `A-19` 이중 조사 결합 [S2] · `E-7` 청자 경어법 일관성 손실 [S2 · estimated]. **본진 보강 4건** — `A-15` 인지·발화 동사 분리 구문 처방 · `A-7` light verb construction 일반화(have/make/take/give) · `F-4` 영어 명사화 접미사(-tion·-ment·-ness·-ity) 통합 · `E-2` 진행형 '~고 있다' 자동 매핑 처방. **본진 hold 1건** — `A-17` 무정물·추상명사 '-들' 부착 [학술 강함, 외부 회차 양성 0건 → NMT 원본 출력 회차 후 재평가]. **post-editese 3축은 metric-only 트랙** — caveat C3(한국어 정량 검증 부재)에 따라 본진 ID 미부여, `metrics_v2.py` 14개 신규 함수로 운영(`deul_overuse_rate` 포함, A-17 hold 검증용). 학술 전문은 외부 SSOT `references/scholarship.md`에 보존(본진 슬림성). valid as of 2026-05.
>
...
### F-6. 무내용 작업 명사화·절차 동사 [S2] · v2.1 신규
- 패턴: 실제 동작을 "작업/수행/진행/실시/처리" 같은 절차어로 감싸 문장을 흐림. 한 문서 3회 이상 또는 한 문장 2회 이상 반복되면 가산.
- 예: "설정 변경을 수행한 후 적용을 진행합니다" → "설정을 바꾼 뒤 적용합니다"
- 예: "성능 측정 작업을 실시합니다" → "성능을 측정합니다"
- 처방: 원문에 있는 행위와 대상만 사용해 명사구를 동사형으로 환원한다. 원문에 없는 주체·장소·조건·수치 보강은 금지한다.
- 제외: "비동기 처리", "일괄 처리", "결제 처리"처럼 `처리`가 도메인 동작명으로 굳어진 경우는 문맥상 실제 의미가 있으면 보존한다.
...
- **v2.1** (2026-06-11): **본진 신규 1건** — `F-6` 무내용 작업 명사화·절차 동사 [S2]. Toss Technical Writing의 문장 중심 원칙 중 "명사 대신 동사", "실제 동작을 드러내기", "불필요한 절차어 줄이기"만 흡수했다. 문서 목적·제목 품질·섹션 구조·개요 작성은 humanize 범위 밖으로 제외. 새 metric은 만들지 않고 taxonomy·playbook·quick-rules·Gemini 인라인 룰만 동기화한다.
