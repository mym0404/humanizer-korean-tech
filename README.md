<p align="center">
  <img src="assets/social-preview.png" alt="Humanizer Korean Tech" width="820">
</p>

# Humanizer Korean Tech — Korean Tech Writing Fork

AI(ChatGPT, Claude, Gemini 등)가 쓴 한글 글에서 AI 티를 줄이고, 문체와 리듬을 자연스러운 한국어로 다듬는 Claude Code/Codex/Gemini용 스킬입니다.
...
전체 설치 옵션과 트러블슈팅은 [INSTALL.md](INSTALL.md)를 참고하세요.
...
설치된 `claude`, `codex`, `gemini` CLI를 자동 감지해 각각 연결합니다. 기본 설치 방식은 심링크라서 저장소를 수정하면 전역 설치본에도 바로 반영됩니다.

### Claude Code

Claude Code에서는 Fast mode와 strict 5인 파이프라인을 모두 사용할 수 있습니다.
...
- `~/.claude/skills/humanizer-korean-tech`
- `~/.claude/skills/humanize`
- `~/.claude/skills/humanize-redo`
- `~/.claude/agents/*.md`

새 Claude Code 세션에서 `/humanizer-korean-tech` 또는 `/humanize`로 실행합니다. 정밀 검증이 필요하면 `--strict`를 붙입니다.

### Codex

Codex는 Fast 단일 호출 모드만 제공합니다.
...
- `~/.codex/skills/humanizer-korean-tech`

Codex에서 `$humanizer-korean-tech`로 실행하거나 `/skills` 메뉴에서 선택합니다.

### Gemini CLI

Gemini CLI도 Fast 단일 호출 모드만 제공합니다.
...
- `gemini extensions link`로 현재 저장소를 확장으로 연결
- `gemini-extension.json`, `GEMINI.md`, `commands/`를 사용

새 Gemini 세션에서 `/humanizer-korean-tech` 또는 `/humanize`로 실행합니다.

### 설치 옵션

| 옵션 | 설명 |
|---|---|
| `./install.sh` | 설치된 Claude/Codex/Gemini를 자동 감지해 모두 연결 |
| `./install.sh --claude-only` | Claude Code만 설치 |
| `./install.sh --codex-only` | Codex만 설치 |
| `./install.sh --gemini-only` | Gemini CLI만 설치 |
| `./install.sh --no-gemini` | Claude/Codex만 설치 |
| `./install.sh --copy` | 심링크 대신 복사 |
| `./install.sh --dry-run` | 실제 변경 없이 설치 작업만 출력 |
| `./uninstall.sh` | 이 저장소를 가리키는 심링크 제거 |

## 사용법

Claude Code, Codex, Gemini에서 윤문할 한글 텍스트를 붙여넣고 요청합니다.
...
```text
/humanizer-korean-tech ./draft.md
```

Codex에서는 `$humanizer-korean-tech ./draft.md`처럼 실행합니다.

옵션은 문장 끝에 자연어로 붙입니다.

```text
/humanizer-korean-tech ./draft.md 장르: 리포트 강도: 보수 최소심각도: S2
```
...
- [ai-tell-taxonomy.md](.claude/skills/humanizer-korean-tech/references/ai-tell-taxonomy.md)
- [rewriting-playbook.md](.claude/skills/humanizer-korean-tech/references/rewriting-playbook.md)
- [quick-rules.md](.claude/skills/humanizer-korean-tech/references/quick-rules.md)
