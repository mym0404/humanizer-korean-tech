...
새 Claude Code 세션에서 `/humanize-korean` 또는 `/humanize`로 실행합니다. 정밀 검증이 필요하면 `--strict`를 붙입니다.
...
Codex에서 `$humanize-korean`으로 실행하거나 `/skills` 메뉴에서 선택합니다.
...
새 Gemini 세션에서 `/humanize-korean` 또는 `/humanize`로 실행합니다.
...
Codex에서는 `$humanize-korean ./draft.md`처럼 실행합니다.
