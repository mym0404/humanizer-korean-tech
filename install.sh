#!/usr/bin/env bash
# Humanizer Korean Tech — Claude Code + Codex CLI + Gemini CLI 전역 설치 스크립트
# 저장소를 클론한 뒤 `./install.sh` 한 번이면 설치된 CLI(claude/codex/gemini)를 자동 감지해
# humanizer-korean-tech 스킬(+ 에이전트)을 전역으로 연결한다. 기본은 심링크(저장소 수정 즉시 반영).
set -euo pipefail
...
Usage: ./install.sh [options]
  ...
  설치된 CLI를 자동 감지해 humanizer-korean-tech 스킬을 전역 설치한다.
  Claude: ~/.claude/skills/{humanizer-korean-tech,humanize,humanize-redo} + ~/.claude/agents/*.md
  Codex : ~/.codex/skills/humanizer-korean-tech
...
if [ "$DO_CLAUDE" != no ] && { [ "$DO_CLAUDE" = yes ] || command -v claude >/dev/null 2>&1; }; then
  ...
  for s in humanizer-korean-tech humanize humanize-redo; do
...
if [ "$DO_CODEX" != no ] && { [ "$DO_CODEX" = yes ] || command -v codex >/dev/null 2>&1; }; then
  ...
  install_one "$REPO/codex/skills/humanizer-korean-tech" "$CODEX_HOME/skills/humanizer-korean-tech"
else
...
if [ "$DO_GEMINI" != no ] && { [ "$DO_GEMINI" = yes ] || command -v gemini >/dev/null 2>&1; }; then
  ...
  if [ "$DRYRUN" = 1 ]; then
    ...
    echo "Y" | gemini extensions link "$REPO" 2>/dev/null && echo "installed: Gemini extension (humanizer-korean-tech)" \
...
echo ""
echo "완료 (mode=$MODE)."
echo "  Claude: 새 세션에서 /humanizer-korean-tech (또는 /humanize)"
echo "  Codex : \$humanizer-korean-tech"
echo "  Gemini: 새 세션에서 /humanizer-korean-tech (또는 /humanize)"
