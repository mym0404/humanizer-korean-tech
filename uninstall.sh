#!/usr/bin/env bash
# Humanizer Korean Tech — 전역 설치 제거 스크립트
...
remove_if_ours "$CODEX_HOME/skills/humanizer-korean-tech" "$REPO/codex/skills/humanizer-korean-tech"
...
if command -v gemini >/dev/null 2>&1; then
  ...
  if [ "$DRYRUN" = 1 ]; then
    echo "+ gemini extensions uninstall humanizer-korean-tech (dry-run)"
  else
    gemini extensions uninstall humanizer-korean-tech 2>/dev/null && echo "removed: Gemini extension (humanizer-korean-tech)" \
